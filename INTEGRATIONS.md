...
**honeypot-mil** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `honeypot-mil` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
honeypot-mil ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
honeypot-mil ... --format json | cognis-connect emit --to misp --url $URL --token $TOK   # MISP event
honeypot-mil ... --format json | cognis-connect emit --to sigma   # Sigma rules
honeypot-mil ... --format json | cognis-connect emit --to splunk --url $URL --token $TOK   # Splunk HEC
```
...
works whatever `honeypot-mil` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="honeypot-mil") for rec in records]   # records = your JSON output

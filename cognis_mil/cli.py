...
def make_cli(tool_name, scan_fn, version="0.1.0", extra_args=None):
    ...
    try:
        ...
        print(f"{tool_name}: error formatting output: {exc}", file=sys.stderr)
        sys.exit(2)
    ...
    if args.out:
        try:
            ...
            print(f"{tool_name}: cannot write to {args.out}: {exc}", file=sys.stderr)
            sys.exit(2)
    else:
        try:
            ...
            # Fall back to lossy ASCII when the terminal cannot handle the output
            # encoding (e.g. Windows cp1252 with emoji in the console formatter).
            safe = out.encode(sys.stdout.encoding or "ascii", errors="replace").decode(

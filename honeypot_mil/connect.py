"""Native cognis-connect emit for honeypot-mil — forward findings to any platform.

Maps honeypot-mil's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    honeypot-mil ... --format json | honeypot-mil-emit --to stix
    honeypot-mil-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = f"HP-{rec.get('type', 'unknown')}-{rec.get('id', 'default')}"
        out['severity'] = 'low'
        out['type'] = 'honeypot'
        out['description'] = f"Event detected on {rec.get('src_ip', 'unknown')} at {rec.get('ts', 'unknown')}"
        out['tags'] = [f"honeypot-{rec.get('type', 'unknown')}", 'unknown']
        out['ipv4'] = rec.get('src_ip')
